...
### Current-product saving point published

- Commit `c991d44` (`feat: complete enrollment portal milestone`) contains the approved current product work and was pushed successfully to `origin/main`. The working tree was clean immediately after the push; root/frontend dependencies, local environment files, secrets, and generated output were not included.
- The Unified Student Records backend implementation is now beginning with failing migration/API tests. MariaDB-backed tests will continue to run sequentially.

### Unified Student Records backend milestone

- Added the student-profile address and requirements-verification fields, pending account-setup state, Laravel-hashed one-time setup codes, setup invitation delivery tracking, and the public password-creation endpoint. Newly provisioned accounts remain disabled until the one-time setup succeeds; existing accounts are backfilled as already set up.
- Added Admission-authorized student directory search by name, student number, or email, protected record viewing, restricted pre-enrollment corrections, automatic curriculum resolution, and invitation resend. Provisioning, invitation delivery, activation, and direct edits now create PII-safe audit records.
- Added student profile-change request creation, revision, cancellation, Admission listing, and in-person approval/rejection. Approval updates the official name, email, and address atomically with stale-profile and duplicate-email protection; rejection preserves the official record. Both decisions notify the student and record PII-safe audits.
- Test-first focused checks have passed for provisioning requirements, forbidden password/curriculum overrides, setup activation and single use, Admission search/view/edit/resend, request submission/revision/cancellation, and approval/rejection. Remaining backend edge cases and the existing provisioning test fixtures are being reconciled before frontend integration.
- Reconciled the legacy provisioning tests with the secure server-generated credential contract and added mail-failure, expired-code, post-enrollment edit, ownership, no-op, duplicate-email, stale-profile, and repeated-decision regressions. The combined backend Student Records slice now passes **28 tests / 213 assertions** sequentially, and `git diff --check` passes.
- Changed the local mail fallback from `log` to the in-memory `array` transport so one-time setup codes are not written to application logs. Production delivery remains environment-configured (for example SMTP) and no credentials are stored in the repository.

### Unified Student Records frontend and contract continuation

- Resumed the approved implementation on the required `main` checkout after confirming the earlier saving point is already on `origin/main`. The new Student Records changes remain uncommitted.
- The remaining work is to finish the pre-enrollment Admission edit controls, rerun the focused backend/frontend regressions, document the new endpoints and data fields, and complete the applicable quality gates.
- Completed the unified Admission workspace, public account-setup page, Student Information workspace, pre-enrollment academic edit controls, and legacy Admission URL redirects. Fresh frontend verification passed 18 feature/service tests, 4 connected-registry tests, 55 portal routing tests, and TypeScript.
- The first combined Vitest run had one worker-start timeout while the other four files passed; the affected service file passed 3/3 when isolated. Registry verification exposed stale baseline magic-count and heading mappings, which were corrected to assert real registry coverage and current accessible headings.
- Fresh sequential backend verification passed 29 Student Records/COR tests before the protected route inventory correctly reported the ten newly introduced endpoints. The inventory and Admission route-boundary assertions were updated; the fresh API surface suite now passes 24 tests / 368 assertions.
- Expanded and validated the OpenAPI 3.1 contract for public account setup, Admission directory/view/edit/resend, Student request/revise/cancel, and Admission decisions. Updated the PRD endpoint inventory and replaced the stale Student Profile dictionary with the current schema, security, lifecycle, COR-address, and retention rules. Redocly and `git diff --check` passed after these documentation changes.
- A scoped-Pint PowerShell selector resolved to an empty file list and therefore formatted the whole backend. The eight unrelated legacy files it touched were identified against the pre-command status snapshot and restored exactly with `apply_patch`; no pre-existing user changes were discarded. The intended Student Records PHP files remain formatted, but their scoped check and the final quality suite still need a fresh run.

### Unified Student Records completion pass

- Fixed three stale docblocks: `StudentProfileResource`'s `@return` shape was missing `name`/`email`/`address`/`program_code`/`program_name`/`requirements_verified_at`/`academic_setup_editable`/`account_setup_status`/`invitation_delivery_status`; `StudentProfilePolicy`'s class comment still claimed Admission Staff "does not read profiles back through this endpoint," contradicting its own `view()`/`viewAny()`; `User`'s model docblock was missing the three `account_setup_*` properties and the `studentProfile` relation.
- Also fixed a stale inline comment in `StoreStudentProfileRequest`: a comment describing the old nullable-and-ignored `curriculum_id` rule was left attached to `entry_year` after `curriculum_id` was tightened to `prohibited` and moved above it.
- Closed the requested test-plan gaps: an explicit account-setup throttle regression (`throttle:10,1` → `429` + `Retry-After` on the 11th attempt); audit-payload PII-absence assertions across provisioning, direct Admission correction, and change-request create/revise/approve/reject (name/email/address/reason/decision-notes text is confirmed absent, not just spot-checked); a new `StudentRecordAccountSetupMigrationTest` proving a pre-existing account is backfilled with `account_setup_completed_at` from its own `created_at`; a frontend test proving the Admission edit dialog hides and rejects the academic-setup selectors (student number/program/entry year/year level/category/financial/admission status) once a student has an enrollment, submitting only the personal fields.
- Fixed real OpenAPI 3.1 inaccuracies: split the shared `official`/`requested` identity schema so only `requested.address` is non-nullable (an official address may still be `null`); added the missing `409 Conflict` response (a new reusable `#/components/responses/Conflict`) to the four change-request endpoints that can throw `ConflictHttpException` — create (already-pending), revise/cancel (no-longer-pending), and decide (no-longer-pending or stale-profile) — and corrected their `422` descriptions, which had wrongly folded those conflict conditions into validation. Redocly passes after every edit.
- PHPStan/Larastan on the full changed-file list found three real `argument.type` errors in `StudentProfileChangeRequestController`: passing `$request->validated()` (typed `array<string, mixed>`) directly into Actions expecting exact array shapes. Fixed by building explicit per-key literal arrays, matching the same pattern `StudentProfileController::store()` already uses. The only remaining PHPStan findings are 13 pre-existing, unrelated errors in `BuildCorSnapshot.php` (fee/currency/date handling) untouched by this slice's one-line `address` change.
- Frontend ESLint on the full changed-file list found 14 real errors, none pre-existing elsewhere in the codebase: `no-base-to-string` from `String(mockCall.body)`/`String(mockCall.url)` on values typed `BodyInit | RequestInfo`, and `no-unsafe-assignment` from `JSON.parse(...)` and `expect.anything()` flowing into typed positions. Fixed using the codebase's established `typeof x === "string"` narrowing convention (matching `enrollment-schedule-card.test.tsx`'s `url()`/body-narrowing idiom) across `account-setup-page.test.tsx`, `admission-provisioning-workspace.test.tsx`, `student-information-workspace.test.tsx`, and `admission-service.(test.)ts`; the `expect.anything()` header assertion was replaced with a direct `Headers.has()` check.
- Found and fixed a real React bug while reviewing accessibility/UX behavior: `ChangeRequestDialog` called `setVerified`/`setNotes` synchronously inside a `useEffect` keyed on the `request` prop (`react-hooks/set-state-in-effect`, a cascading-render anti-pattern). Replaced with a `key={selected?.id ?? "closed"}` remount on the dialog instead, which resets local state for free and removed the effect entirely.
...
- Two new project memories were saved: the queue_tickets migration-reversibility bug (this session), alongside the existing `ProgramVisibilityTest` bug memory it turned out to cascade into during the full-suite run.
- Working tree remains uncommitted on `main` per instructions; no commit or push occurred.

## 2026-08-26 — Dev-database sync, a MariaDB crash recovery, and a structured-name feature

### Dev database out of sync with the completed Student Records work

- Live account creation on the running dev server failed with a generic "Account not created" error. `storage/logs/laravel.log` showed the real cause: `SQLSTATE[42S22]: Column not found: 'account_setup_completed_at'` — the two new Student Records migrations had only ever been applied to `grc_enrollment_test` during this session's test runs, never to the actual dev database (`grc_enrollment`) the running API server uses.
- Running `php artisan migrate` against the dev connection failed differently: `grc_app` (the app's own DB user) has DML-only privileges by design (ADR 0007), so DDL must go through the dedicated `mariadb_migrator` connection documented in `docs/runbooks/mariadb-local.md`.

### MariaDB crashed mid-migration; full recovery performed
...
- User request: split the single "Full name" field into First name / Last name / Middle initial (optional) / Suffix (optional, Jr./Sr./III/…), applied to the whole `users` table (shared by all 9 roles) and to Faculty too. Scoped with the user first: confirmed no Professor account-creation UI should be built (none exists today — Faculty accounts are seeder-only), and that middle name should store just an initial, not the full name.
- Architecture: added nullable `first_name`/`middle_initial`/`last_name`/`suffix` columns to `users` (migration `2026_08_26_000003_split_user_name_into_parts`) while leaving the existing `name` column as a stored, app-synced composed display value (`App\Domain\Identity\PersonName::compose()`) — not a DB-generated column, and deliberately not backfilled/rewritten during migration. This kept the change additive: every existing Resource, search query (`WHERE name LIKE`), audit payload, COR document, and frontend display that reads `name` kept working unmodified, and only the two flows that actually collect a name (Student provisioning/correction, Student self-service change requests) needed updating to send/compose the parts. Nullable at the DB level (not required) specifically so Faculty and other seeder-created rows — which don't go through a split-collecting form — keep working unchanged; required only at the Form Request layer for the flows that do collect it.
- Backfilled all 3,872 existing users (via `chunkById`, not `each()`, learning directly from the COM/CATR bug above) with a best-effort parser handling plain "First Last"/"First Middle Last" names, trailing `Jr./Sr./II/III/IV/V` suffixes, comma-formatted "SURNAME,GIVEN NAME(annotation)" faculty CSV-import artifacts, and single-token names — all without touching the existing `name` value. Reworked `student_profile_change_requests.requested_name` into the same four-part shape (table was empty in the dev DB, so its migration was edited in place and rerun rather than layering another migration on top).
- Updated end-to-end: `StoreStudentProfileRequest`/`UpdateStudentProfileRequest`/`StoreStudentProfileChangeRequestRequest` validation; `ProvisionStudent`, `UpdateStudentProfile` (merges partial-PATCH name parts with existing stored parts before recomposing), `ManageStudentProfileChangeRequest` (create/revise/no-op comparison now compares parts, not the composed string), `DecideStudentProfileChangeRequest` (approval writes both the parts and the recomposed `name`); `StudentProfileResource` and `StudentProfileChangeRequestResource` (official/requested) now expose the parts alongside the composed `name`; Zod schemas and the Create Account / Admission edit dialog / Student self-service request form all replaced their single name input with First name, Last name, Middle initial, and Suffix fields. OpenAPI and both data dictionaries updated; Redocly passes.
- Verification: new `PersonNameTest` (6 cases) and `SplitUserNameMigrationTest` (7 parsing edge cases, 24 assertions) both pass; full focused backend suite **67 tests / 673 assertions** passes, including a real regression this change caused and fixed — `StudentRecordAccountSetupMigrationTest` used a hardcoded `--step=2` rollback that silently stopped rolling back one migration short once the new name-split migration became the most recent one, so its "column removed" assertion was actually checking a column that was still there. Frontend: TypeScript, scoped ESLint/Oxlint/Prettier, and **74 Vitest tests** pass; production `next build` succeeds. Applied to the dev database (`grc_enrollment`) last, after MariaDB's health was reconfirmed; final row check: 3,872/3,872 users have `first_name` populated, `name` unchanged for every existing account.
- Not done, out of scope by explicit user decision: no Professor/Faculty account-creation or -editing UI was built; existing seeders were not updated to populate the new split columns for newly-seeded rows (they remain nullable there by design).

## 2026-08-27 — Account-setup email redesign, attrition analytics fix, prospectus visibility, and irregular-student eligible-subject scoping

- Redesigned `resources/views/mail/student-account-setup.blade.php` into a formal, table-based inline-styled HTML email (branded header band, styled CTA button, boxed one-time setup code, expiry notice, footer disclaimer) at the user's request, after confirming via a live test send that Gmail SMTP delivery itself was already working.
- Fixed the Registrar's attrition analytics endpoint, which 422'd on every real request: `IndexAttritionRequest`'s validator compared `AcademicTerm::semester` against the literal strings `'1st semester'`/`'2nd semester'`, but every real term (per `AcademicTermSeeder`/`DemoEnrollmentSeeder`) stores the short form `'1st'`/`'2nd'` — so the "baseline/comparison must be the 1st and 2nd semester of the same school year" check always failed. The existing test masked this because its own fixture used the same wrong-but-matching long form. Fixed both the validator and the test fixture to the real short form; live-verified against the dev database with a temporary Sanctum token (revoked after).
- Fixed prospectus visibility for students with no recorded grades yet: `AcademicRecordView` gated its entire body — including the "Prospectus" button/dialog — behind `isEmpty={(record) => record.terms.length === 0}`, so a student with zero grades saw only a bare "No grades have been recorded yet" message and could never reach their prospectus (the full curriculum mapping of subjects still needed). Removed that `isEmpty` gate so the body always renders once the query resolves; the existing per-section fallbacks already handle the zero-terms case gracefully, with an improved message directing the student to the Prospectus button.
- Scoped the eligible-subjects pool for irregular students to their current year-level standing, at the user's explicit request ("irregular students should only see what they still need based on year level, not the whole 1st–4th year curriculum at once"): `BuildEligibleSubjectPool` now bounds an irregular student's placements to backlog-or-current using the same curriculum-ordinal rule `ClassifyEnrollmentStanding` already uses to decide what "needs adding" (a placement whose `SemesterCoverage::primary()->ordinal()` is ahead of the student's current-term ordinal is excluded from the pool entirely). Regular/unclassified students are unaffected — their pool is still the full curriculum, already implicitly bounded by `BlockSectionAccessPolicy`. 3 new focused tests added to `EligibleSubjectsEndpointTest` (irregular student excludes a future-year subject, irregular student still sees an earlier-year backlog subject, regular student's pool remains unbounded); full file passes (32 tests, 73 assertions). Backend PHPStan and Pint clean on touched files (one pre-existing, unrelated PHPStan finding confirmed present before this session's changes). Frontend TypeScript, ESLint, and the related Vitest suites (30 tests) pass.
- No commit or push made; working tree remains on `main` per standing instructions.

## 2026-08-27 — Freshman/Transferee student type at admission

- User request: Admission needs to record whether the enrolling student is a Freshman or a Transferee ("yung sa admission need natin ilagay if freshmen or transferee yung nag e-enroll"). Added `App\Domain\Identity\StudentType` (`freshman`/`transferee`, PROVISIONAL VOCABULARY per PRD §17, same pattern as `FinancialStatus`) and a new nullable `student_profiles.student_type` column (migration `2026_08_27_000001_add_student_type_to_student_profiles.php`, applied to the dev database). Informational only — does not itself grant `TransfereeCredit` rows; Registrar staff still record those separately per subject.
- Required at Admission provisioning (`StoreStudentProfileRequest`), optional on later corrections (`UpdateStudentProfileRequest`), locked once the student has an enrollment (added to `UpdateStudentProfile::ACADEMIC_SETUP_FIELDS`, same as `enrollment_category`/`financial_status`). Exposed via `StudentProfileResource` as `student_type`/`student_type_label`. Wired into the Student Records "Create a student account" form (new required Select, default Freshman) and the record-correction dialog, plus a new "Type" column in the Student Directory table.
- Along the way, found and fixed a pre-existing, unrelated broken test file (`ProvisionStudentAuditTest.php`, 2 of 4 tests failing before this change): its provisioning payloads still used the pre-name-split `name`/`password`/`curriculum_id` fields, which the current `StoreStudentProfileRequest` contract rejects (`password`/`curriculum_id` are `prohibited`; `first_name`/`last_name` are `required`) — confirmed via `git stash` that this predates the `student_type` work. Corrected the payloads to the real contract and fixed the audit-log lookup to filter by `STUDENT_PROFILE_PROVISIONED` (a successful provisioning now also writes a second, separate invitation-sent audit row).
- Verification: full student-profile-related backend suite passes — **63 tests / 659 assertions** across `StudentProfilesEndpointTest`, `AdmissionStudentRecordsEndpointTest`, `ProvisionStudentAuditTest`, `StudentProfilePolicyTest`, `ApiSurfaceTest`, `StudentProfileChangeRequestsEndpointTest`. Backend PHPStan and Pint clean on every touched file (a repo-wide Pint pass shows only pre-existing, unrelated drift in files this change never touched). Frontend: TypeScript and ESLint clean; **9 Vitest tests** pass (`admission-service.test.ts`, `admission-provisioning-workspace.test.tsx`, `student-information-workspace.test.tsx`). OpenAPI updated (`StudentProfileResource`, `StoreStudentProfileRequest`, `UpdateStudentProfileRequest`) and Redocly-validated. Data dictionary (`student-profile-foundation.md`) updated with the new column.
- No commit or push made; working tree remains on `main` per standing instructions.
...
## 2026-08-27 — Registrar Head "Invite Staff": account creation by email for every non-Student role

- User request: Registrar Head needs a function to input someone's email and have it create an account for them, for every user role — mirroring the existing "input email, send setup invitation with a one-time code" pattern. Clarified scope with the user via AskUserQuestion first: Registrar Head may invite any staff/leadership role (Faculty, Program Chair, Dean, Executive Director, Registrar Head, Registrar Staff, Accounting Staff, IT Control) but not Student (Admission's existing "Create a student account" already owns that, with required program/curriculum/entry-year fields no other role has) or AdmissionStaff.
- Investigation before building found this pattern was **already implemented once**, for exactly one role: Program Chair's existing (uncommitted, from an earlier session) "Invite Professors" feature — `InviteFacultyAccount`/`SendFacultyAccountSetupInvitation`/`ActivateFacultyAccount`/`FacultyInvitationController`, an `AccountSetupPage` `variant` prop, a `faculty-account-setup` page — creates a placeholder-named disabled account from just an email, sends a setup email, and the invitee supplies their own name when redeeming the one-time code (unlike the Student flow, where Admission collects the full name upfront). Rather than duplicate a differently-shaped flow, this new feature was built as a parallel, structurally identical set of files spanning every allowed role instead of one, reusing this exact established pattern: `UserRole::registrarInvitableCases()` (the 8-role list), `InviteStaffAccount`/`SendStaffAccountSetupInvitation`/`ListStaffInvitations`/`ActivateStaffAccount`, `StaffAccountSetupMail` (role-label-parametrized), `StaffInvitationController`/`StaffAccountSetupController`, new `STAFF_ACCOUNT_*` audit actions and a `STAFF_ACCOUNT` auditable type, routes gated `role:registrar_head` (`GET/POST /staff-invitations`, `POST /staff-invitations/{user}/resend`) plus a public throttled `POST /auth/staff-account-setup`. The Program Chair's existing Faculty-only flow was left completely untouched — Registrar Head's reach is role-spanning and not college-scoped, a genuinely different authorization shape.
- Frontend: new `StaffInvitationWorkspace` (mirrors `FacultyInvitationWorkspace`, plus a per-queued-invite Role select since invites now span 8 roles, not one) registered as Registrar Head's new "Invite Staff" module; `AccountSetupPage`'s `variant` prop extended from `"student" | "faculty"` to add `"staff"` (a `COPY` lookup replaced the growing binary ternaries); new `/staff-account-setup` page and route guard layout.
- Along the way, found and fixed a **pre-existing OpenAPI spec bug** in the (also uncommitted) Faculty invitation documentation: `FacultyInvitation`'s `invitation_sent_at`/`activated_at` used OpenAPI 3.0's `nullable: true`, invalid under this spec's declared 3.1 — Redocly only surfaced it once the new, structurally identical `StaffInvitation` schema was added alongside it. Fixed both to the correct 3.1 `type: [string, 'null']` array syntax.
- Verification: new `StaffInvitationsEndpointTest` (13 tests / 66 assertions — every allowed role invitable, Student/AdmissionStaff/QueueKiosk rejected, duplicate email, cross-role list, resend, mail-failure handling, full activation with own-name capture, invalid code, a Student email rejected even with a well-formed request) plus `ApiSurfaceTest` extended for the 4 new routes and their exact role gate (25 tests total). Frontend: `StaffInvitationWorkspace` (6 tests), `AccountSetupPage` extended for the "staff" variant (5 tests), `module-registry`/`role-capabilities`/`portal-module-page` exhaustive-coverage tests updated for the new connected module (67 tests). PHPStan and Pint clean on every new/touched backend file; TypeScript and ESLint clean on every new/touched frontend file. OpenAPI validated (Redocly).
- No commit or push made; working tree remains on `main` per standing instructions.
