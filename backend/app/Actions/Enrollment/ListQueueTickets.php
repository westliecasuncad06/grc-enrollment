 ...
 * effective order (`COALESCE(requeued_at, created_at)`) -- plain arrival
 * order for a never-skipped ticket, or requeue order for one that was
 * skipped. §17 leaves any reset or priority policy unconfirmed, so no
 * priority-tier ordering is asserted at this list level (the Cashier's own
 * waiting-line display sorts by priority tier itself; see `byQueueOrder`
 * in `accounting-payment-workspace.tsx`).
 *
