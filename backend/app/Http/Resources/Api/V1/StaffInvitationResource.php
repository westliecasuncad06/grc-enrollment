<?php

namespace App\Http\Resources\Api\V1;

use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

/**
 * One account across the roles a Registrar Head may invite, invited or
 * already active — mirrors FacultyInvitationResource, plus the role itself
 * since this list spans several.
 *
 * @property-read User $resource
 */
final class StaffInvitationResource extends JsonResource
{
    /**
     * @return array{
     *     type: string,
     *     id: int,
     *     email: string,
     *     name: string,
     *     role: string,
     *     role_label: string,
     *     status: string,
     *     invitation_sent_at: ?string,
     *     activated_at: ?string
     * }
     */
    public function toArray(Request $request): array
    {
        return [
            'type' => 'staff_invitation',
            'id' => $this->resource->id,
            'email' => $this->resource->email,
            'name' => $this->resource->name,
            'role' => $this->resource->role->value,
            'role_label' => $this->resource->role->label(),
            'status' => $this->status(),
            'invitation_sent_at' => $this->resource->account_setup_invitation_sent_at?->toIso8601String(),
            'activated_at' => $this->resource->account_setup_completed_at?->toIso8601String(),
        ];
    }

    private function status(): string
    {
        if ($this->resource->account_setup_completed_at !== null) {
            return 'activated';
        }
        if ($this->resource->account_setup_invitation_failed_at !== null) {
            return 'failed';
        }

        return 'pending';
    }
}
